...
export default class CefFocus {
    static setup(editor) {
        let renderFocusCaret = Throttler.first(() => {
            ...
            if (!editor.removed && editor.getBody().contains(document.activeElement)) {
                ...
                if (rng.collapsed) {
                    let caretRange = CefUtils.renderRangeCaret(editor, editor.selection.getRng(), false);
                    editor.selection.setRng(caretRange);
                }
            }
        }, 0);
        editor.on("focus", () => renderFocusCaret.throttle());
        editor.on("blur", () => renderFocusCaret.cancel());
    }
}